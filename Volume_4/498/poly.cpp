// You have to use Horner's method to evaluate polynomial efficiently.
...
string horner(const vector<string>& coeffs, const string& x) {
  string result = coeffs[0];
  for (int i = 1; i < coeffs.size(); ++i) {
    result = bigadd(coeffs[i], bigmul(result, x));
  }
  return result;
}
