...
pair<string, string> bigdiv(const string& s1, const string& s2) {
  ...
  // doing binary search is correct
  string lb = "1", rb = s1;
...
int main() {
    ...
    while (cin >> N >> F) {
        ...
        for (int i=0;i<N;++i) {
            getline(cin, line);
            total = bigadd(total, line);
        }
        string quo, rem;
        tie(quo, rem) = bigdiv(total, to_string(F));
        cout << "Bill #" << cases << " costs " << total << ": each friend should pay " << quo << endl;
        cout << endl;
        cases++;
    }
