...
tuple<string, int> DFS(const vector<char>& list, const string& prev, STNode* node) {
  if (node->end == oo) {  // leaf
    return make_tuple("", 1);
  }
  ...
  string current = prev +
    string(list.begin() + node->start, list.begin() + node->end);
  ...
  for (int i = 0; i < CHAR_SIZE; ++i) {
    if (node->edge[i] != nullptr) {
      string ts;
      int tc;
      tie(ts, tc) = DFS(list, current, node->edge[i]);
      cnt += tc;
      if (tc > 1 &&  ts.size() > bs.size()) {
        bs = ts;
        bc = tc;
      }
    }
...
void Solve(const string& str) {
  ...
  string seq;
  int cnt;
  tie(seq, cnt) = DFS(st.list, "", st.root);
  if (cnt == 1 || seq == "") {
